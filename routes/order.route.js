...
orderRoute.post("/placeorder",async(req,res)=>{
    ...
    // handling the case where cart is empty
    if(cart.length===0){
        res.status(200).send({msg:"Empty.Please add item to the cart before placing the order!"})
    }else{
...
orderRoute.get("/orderdetail/:orderid",async(req,res)=>{
    try {
        ...
        res.status(500).send({msg:error.message}) // catching the error
    }
})
